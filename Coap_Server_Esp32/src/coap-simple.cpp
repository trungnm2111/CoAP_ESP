...
Coap::Coap(
    ...
    this->_udp = &udp;
    this->coap_buf_size = coap_buf_size;                    // Kích thước bộ đệm mặc định là COAP_BUF_MAX_SIZE
    this->tx_buffer = new uint8_t[this->coap_buf_size];     // Khởi tạo bộ đệm truyền với kích thước coap_buf_size
    this->rx_buffer = new uint8_t[this->coap_buf_size];     // Khởi tạo bộ đệm nhận với kích thước coap_buf_size
}


/* Hàm này được gọi khi đối tượng Coap được hủy đảm bảo giải phóng bộ nhớ được cấp phát 
cho tx_buffer và rx_buffer 
Hàm này xảy ra khi 
Đối tượng Coap ra khỏi phạm vi (scope) của nó, ví dụ như khi một biến cục bộ của một hàm kết thúc.
Đối tượng Coap được hủy thủ công bằng cách sử dụng từ khóa delete nếu nó được cấp phát động (dynamic allocation) bằng từ khóa new.
Đối tượng Coap là một phần của một đối tượng khác và đối tượng cha bị hủy.
Destructors đảm bảo rằng bất kỳ tài nguyên nào được cấp phát trong quá trình sống của đối tượng đều được giải phóng đúng cách để tránh rò rỉ bộ nhớ. 
*/
Coap::~Coap() {
...
bool Coap::loop() {
    int32_t packetlen = _udp->parsePacket(); // Tính toán kích thước gói tin UDP

    while (packetlen > 0) {
        // Tiến hành đọc dữ liệu từ bộ đệm UDP và lưu vào rx_buffer với kích thước là coap_buf_size
        packetlen = _udp->read(this->rx_buffer, packetlen >= coap_buf_size ? coap_buf_size : packetlen);
        // Khởi tạo một gói tin CoapPacket
        CoapPacket packet;
        // Kiểm tra kích thước gói tin có lớn hơn kích thước header của gói tin CoAP không
        // Nếu không hoặc không phải là gói tin CoAP thì tiếp tục đọc dữ liệu từ bộ đệm UDP
        // parse coap packet header
        if (packetlen < COAP_HEADER_SIZE || (((this->rx_buffer[0] & 0xC0) >> 6) != 1)) {
            packetlen = _udp->parsePacket();        // Tiếp tục đọc dữ liệu từ bộ đệm UDP
            continue;
        }
        /* Đoạn mã này trích xuất các thông tin từ rx_buffer và
        lưu chúng vào các trường tương ứng của cấu trúc packet,
        bao gồm loại gói tin (type), độ dài token (tokenlen), mã (code),
        và ID thông điệp (messageid). */
        ...
        /*
        Kiểm tra xem kích thước của header CoAP cộng với độ dài của token có nhỏ hơn
         tổng độ dài của gói tin (packetlen) hay không. Điều này đảm bảo rằng có đủ
          dữ liệu để chứa các tùy chọn (options) và payload.
        */ 
        if (COAP_HEADER_SIZE + packet.tokenlen < packetlen) {
            int optionIndex = 0;            // Chỉ số tùy chọn hiện tại
            uint16_t delta = 0;             // Biến để lưu trữ giá trị delta của tùy chọn
            uint8_t *end = this->rx_buffer + packetlen;     // Con trỏ đến cuối của gói tin
            uint8_t *p = this->rx_buffer + COAP_HEADER_SIZE + packet.tokenlen;   // Con trỏ đến vị trí bắt đầu từ vị trí sau hearder và token
            /* Vòng lặp phân tích các tùy chọn trong gói tin */
            while(optionIndex < COAP_MAX_OPTION_NUM && *p != 0xFF && p < end) {     // Đảm bảo ko vượt quá số lượng tùy chọn tối đa và chưa gặp kí tự kết thúc tùy chọn
            ...
            if (p+1 < end && *p == 0xFF) {
                ...
                storePacketData(packet.payload, packet.payloadlen);              
            } else {
                packet.payload = NULL;
                packet.payloadlen= 0;
            }
...
void Coap::storePacketData(const uint8_t *payload, size_t payloadlen) 
{
    if (packetCount < COAP_MAX_CALLBACK) {
        jsonpackets[packetCount].data = new uint8_t[payloadlen];
        memcpy(jsonpackets[packetCount].data, payload, payloadlen);
        jsonpackets[packetCount].len = payloadlen;
        packetCount++;
    } else {
        // Giải phóng bộ nhớ của gói tin đầu tiên
        delete[] jsonpackets[0].data;

        // Dịch chuyển các gói tin còn lại lên một vị trí
        for (size_t i = 1; i < COAP_MAX_CALLBACK; i++) {
            jsonpackets[i - 1] = jsonpackets[i];
        }

        // Lưu gói tin mới vào vị trí cuối cùng
        jsonpackets[COAP_MAX_CALLBACK - 1].data = new uint8_t[payloadlen];
        memcpy(jsonpackets[COAP_MAX_CALLBACK - 1].data, payload, payloadlen);
        jsonpackets[COAP_MAX_CALLBACK - 1].len = payloadlen;

    }
}

void Coap::getPacketData(CoapPacketData *packet_out)
{
    packet_out->data = NULL;
    packet_out->len = 0;
    size_t index = packetCount - 1;
    packet_out->data = jsonpackets[index].data;
    packet_out->len = jsonpackets[index].len;
}
void Coap::clearPackets() {
    for (int i = 0; i < packetCount; i++) {
        if (jsonpackets[i].data != NULL) {
            delete[] jsonpackets[i].data;
            jsonpackets[i].data = NULL;
        }
        jsonpackets[i].len = 0;
    }
    packetCount = 0;
