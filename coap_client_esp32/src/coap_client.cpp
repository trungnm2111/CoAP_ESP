...
uint16_t Coap::sendPacket(CoapPacket &packet, IPAddress ip, int port) {
    ...
    *p++ = (packet.messageid & 0xFF);
    ...
    if (packet.payloadlen > 0) {
        if ((packetSize + 1 + packet.payloadlen) >= (unsigned int)coap_buf_size) {
            return 0;
        }
        *p++ = 0xFF;
...
// make
uint16_t Coap::Delete(IPAddress ip, int port, const char *url) {
    return this->send(ip, port, url, COAP_NONCON, COAP_DELETE, NULL, 0, NULL, 0);
}

uint16_t Coap::get(IPAddress ip, int port, const char *url) {
    return this->send(ip, port, url, COAP_NONCON, COAP_GET, NULL, 0, NULL, 0);
}
...
// write test or unit test for function

uint16_t Coap::ping(IPAddress ip, int port) {
    CoapPacket packet;
    packet.type = COAP_CON;            // Ping thường dùng Confirmable
    packet.code = 0x00;                // CoAP ping có code 0x00
    packet.token = NULL;               // Không cần token
    packet.tokenlen = 0;
    packet.payload = NULL;             // Không có payload
    packet.payloadlen = 0;
    packet.optionnum = 0;
    packet.messageid = rand();         // Sử dụng ID ngẫu nhiên

    return this->sendPacket(packet, ip, port);  // Gửi gói tin ping
}
...
uint16_t Coap::send(IPAddress ip, int port, const char *url, COAP_TYPE type, COAP_METHOD method, const uint8_t *token, uint8_t tokenlen, const uint8_t *payload, size_t payloadlen, COAP_CONTENT_TYPE content_type, uint16_t messageid) {
    ...
    char ipaddress[16] = "";
    sprintf(ipaddress, "%d.%d.%d.%d", ip[0], ip[1], ip[2], ip[3]); // hàm này để chuyển địa chỉ IP thành chuỗi kí tự
...
bool Coap::loop() {
    ...
    while (packetlen > 0) {
        ...
        // Serial.println((const char*)this->rx_buffer);
        CoapPacket packet;
...
void callback_response(CoapPacket &packet, IPAddress ip, int port) 
{
  ...
  Serial.print("CoAP Server Response: ");
  Serial.println(p);
}
